-- quizzes テーブルを作成し、quiz01〜quiz05 のサンプルデータを投入します
-- 実行: psql "$DATABASE_URL" -f migrations/quizzes.sql
CREATE TABLE IF NOT EXISTS quizzes (
  quiz_id TEXT PRIMARY KEY,
  title TEXT,
  data JSONB, -- 質問/選択肢/正解番号などを JSON で格納
  reward INTEGER DEFAULT 100,
  is_ex BOOLEAN DEFAULT FALSE
);

-- 例: quiz01..quiz05（サンプル問題を入れています。必要に応じて編集してください）
INSERT INTO quizzes (quiz_id, title, data, reward, is_ex)
VALUES
